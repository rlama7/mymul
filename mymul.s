...
mymul:
    ...
    addiu $sp, $sp, -20			# pushes stack to create space for 20 bytes word space
    sw $ra, 16($sp)				# store return address
    #
    # store left and right multilicand above the $sp space created
    sw $a0, 20($sp)				# store left multiplicand at $a0
    sw $a1, 24($sp)				# store right multiplicand at $a1
	#    
    #sw $v0, 16($sp)	# store retval
	#       	
    ...
    li $t0, 1 						# $t0 = 1 
    lw $a0, 20($sp)				# load $a0
    bne $a0, $t0, LableI			# branch if ($a0 !== 1)  
	#    
    lw $a1, 24($sp) 				# load $a1
    move $v0, $a1 				# $v0 = $a1
    b Exit    			
...
LableI:
	...
	lw $a1, 24($sp)				# load $a1
	jal mymul 						# jump and link mymul
	#	
	# Every time jump is made we need to load argument register
	lw $a1, 24($sp)				# load $a1
	add $v0, $v0, $a1			# $v0 = $v0 + $a1
	#		
Exit:
	lw $ra 16($sp)				# load return address
	addiu $sp, $sp, 20 			# pop $sp to free up space we used
	jr $ra								# jump return
	#
 ####################End of my code	
